function Price = PricingFDMImpl(S0, K, RiskFree, Dividend, Volatility, DayCount, BasedYears, NumOfTGrid, NumOfSGrid, Type, PlotFlag, NumFig)
Smax = 5 * S0;
...
a = 0.5 * dt*((RiskFree - Dividend)/dS - Volatility.^2 .* Strikes./dS.^2) .* Strikes;
b = 1 + dt * (Volatility^2 .* Strikes.^2/(dS.^2) + (RiskFree - Dividend));
c = - 0.5 * dt*((RiskFree - Dividend)/dS + Volatility.^2 .* Strikes./dS.^2).* Strikes;

coeff = diag(a(3:NumOfSGrid),-1) + diag(b(2:NumOfSGrid)) + diag(c(2:NumOfSGrid-1),1);
[L,U] = lu(coeff);
Boundary = zeros(NumOfSGrid - 1,1);
for j = NumOfTGrid - 1 : -1 : 1
   Boundary(1) = - a(2) * payoff(1,j);
   payoff(2:NumOfSGrid, j) = U \ (L \ (payoff(2:NumOfSGrid, j + 1) + Boundary));
end
if PlotFlag == 1
    figure(NumFig)
